  ...
  if (variant === "trending") {
    return (
      ...
      >
        ...
        <div className="flex items-center opacity-100 transition-opacity md:opacity-0 md:group-hover:opacity-100" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex items-center justify-end opacity-100 transition-opacity md:opacity-0 md:group-hover:opacity-100" onClick={(e) => e.stopPropagation()}>
          {canShowPlaylist && <SongActions song={song} dropdownPosition="top" />}
        </div>
      </div>
    )
  }
  ...
  return (
    ...
    >
      <div className="relative aspect-square w-full">
        <div className={cn(
          ...
          <div
            className={cn(
              ...
              isPlaying ? "opacity-100 bg-[rgba(6,182,212,0.35)]" : "opacity-0 group-hover:opacity-100 bg-black/40"
            )}
          >
            {isPlaying && (
              <div className="flex flex-col items-center gap-1.5">
                ...
                <span className="text-accent text-[10px] font-semibold tracking-widest uppercase">{t("songCard.playing")}</span>
              </div>
            )}
          </div>
          {/* Bottom gradient */}
        ...
        {/* Action buttons — z-30 ensures they're always on top and clickable */}
        <div
          className="absolute right-3 bottom-3 z-30 flex items-center gap-1 opacity-100 transition-all duration-200 md:opacity-0 md:group-hover:opacity-100"
